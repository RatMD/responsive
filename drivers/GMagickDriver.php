...
class GMagickDriver implements GraphicDriver
{
    ...
    static public function getSupportedFormats(): array
    {
        ...
        $available = \Gmagick::queryFormats();
        foreach ($keys AS $key) {
    ...
    /**
     * Create a new GMagick driver instance
     *
     * @param string $source
     */
    ...
    public function getDimensions(): ?array
    {
        ...
        if ($this->image === null) {
            $this->image = new \Gmagick($this->source);
        }
    ...
    public function convert(string $dest, array $options): bool
    {
        ...
        if (!empty($options['width']) && !empty($options['height'])) {
            $image->resizeImage($options['width'], $options['height'], \Gmagick::FILTER_LANCZOS, 1);
        }
